-- Display Performance each funding officer based on DI321PN Current Account Balance (CSV Format from DWH BRISIM)
...
-- 5:44 18 November 2019, Rawamangun

-- Input : Format Column CSV
--  1. periode
--  2. uker code
--  3. curr code
--  4. curr desc
...
res, ReportFileName1, ReportFileName2 = iup.GetParam("Pick Reports DI321PN  in CSV Format (Source: BRISIM)", nil, [=[
Current Report  : %f[OPEN|*.csv;*.txt|CURRENT|NO|NO]\n
Baseline Report : %f[OPEN|*.csv;*.txt|CURRENT|NO|NO]\n
]=]
,'','')
--,"G:\\1\\LUA Script\\Raw Data\\DI321 PN PENGELOLAH 0386 20190905.csv","G:\\1\\LUA Script\\Raw Data\\DI321 PN PENGELOLAH 0386 20181231.csv")
...
--				6	Account Officer
--				7	Remark
				list_acc[acc_no] = {acc_cif, acc_name, 0, acc_balance, acc_balance, acc_officer, "Giro baru"}
			end
		...
		if f[1] ~= "" then
			...
			if INCLUDE_IA or (string.sub(acc_no,-3,-3) ~= "9") then
				if list_acc[acc_no] then
					...
					acc_officer = f[17]
					list_acc[acc_no] = {acc_cif, acc_name, acc_balance, 0, -acc_balance, acc_officer, "Penutupan Giro"}
				end
...
<title>BRI Reporting Tool: Giro DI321PN</title>
<style type="text/css"> 
...
table td.plus {
 ...
 fo2:write([=[ 
 <h2>&nbsp;&nbsp;&nbsp;Summary Giro Performance Report (Periode ]=]..posisi_report2..[=[ - ]=]..posisi_report1..[=[)</h2>
 <table cellspacing='0'>
	<thead>
		<tr>
			<th>Personal No</th>
...
for k, v in pairs(list_summ) do
	...
	--print(k, v[1],v[2],v[3],v[4],v[5]) --debug
end
fo2:write([=[
	</tbody>
 </table>
